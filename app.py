...
# Load machine learning pipeline components safely
@st.cache_resource
def load_assets():
    model = joblib.load('assets/macro_model.pkl')
    tfidf = joblib.load('assets/tfidf.pkl')
    prod_features = joblib.load('assets/product_features.pkl')
    defaults = joblib.load('assets/macro_defaults.pkl')
    return model, tfidf, prod_features, defaults

try:
    macro_model, tfidf, product_features, macro_defaults = load_assets()
except Exception as e:
    st.error("Model assets not found. Please ensure the 'assets/' folder contains all serialized .pkl files.")
    st.stop()
